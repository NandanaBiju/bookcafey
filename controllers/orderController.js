...
const returnRequest = async (req, res) => {
  try {
    ...
    if (reason === "Received a broken/damaged item") {
      // If reason is "Received a broken/damaged item", do not update stock
      ...
      // If reason is not "Received a broken/damaged item", update stock
      const findOrder = await Order.findByIdAndUpdate(
        { _id: id },
        {
          $set: {
...
const cancelReturn = async (req, res) => {
  try {
    ...
    // Update stock for each item in the canceled return order
    for (let i = 0; i < findOrder.items.length; i++) {
      const productId = findOrder.items[i].productId;
      const quantity = findOrder.items[i].quantity;

      // Decrease the countInStock by the quantity of the item
      await Product.findByIdAndUpdate(
        { _id: productId },
        {
          $inc: {
            countInStock: -quantity, // Subtract the quantity from countInStock
          },
        }
      );
    }
